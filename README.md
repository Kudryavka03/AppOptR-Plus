...
规则页和调优页的每个应用都有“克隆规则”按钮。选择来源后输入新的包名，可按需
勾选以下大类，而不必手工重建配置：
...
内存组选项只复制应用对既有内存组的引用，不会复制或创建内存组定义，因此
{uid}、{pid}、{pkg} 模板会为新应用重新展开。为防止静默覆盖，目标包名已有
所选同类规则时，克隆会被拒绝；未选择的目标规则保持不变。

新增 CPU 规则或调优应用时，包名输入框获得焦点便会优先显示已存在于“规则”页和
“调优”页的包名，随后才是已安装或正在运行的应用；继续输入可按名称筛选。
